      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-10">
        ...
        <div className="mt-12 grid md:grid-cols-2 gap-5">
          {projects.map((p, i) => (
            <article
              key={p.title}
              className="hover-lift group relative rounded-xl border border-white/10 bg-zinc-950/40 p-6 md:p-7 hover:border-teal-300/40 overflow-hidden"
            >
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <ArrowUpRight className="h-5 w-5 text-zinc-500 group-hover:text-teal-300 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 transition-all" />
              </div>

              <h4 className="mt-4 font-display text-2xl text-zinc-100 group-hover:text-teal-200 transition-colors">
                {p.title}
              </h4>
              ...
              <ul className="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs text-zinc-300 font-mono">
                {p.bullets.map((b) => (
                  <li
                    key={b}
                    className="px-2.5 py-1.5 rounded-md border border-white/5 bg-white/[0.02]"
                  >
                    {b}
                  </li>
                ))}
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                {p.tags.map((t) => (
                  <span
                    key={t}
                    className="font-mono text-[11px] px-2 py-0.5 rounded border border-teal-300/20 text-teal-300/90 bg-teal-300/5"
                  >
                    {t}
                  </span>
                ))}
